...
export async function POST() {
  try {
    ...
    if (existingCount > 0) {
      return NextResponse.json(
        { error: "Your inventory already has items. Clear existing items first or add items individually." },
        { status: 400 }
      );
    }

    // Bulk insert all grocery items
    ...
    return NextResponse.json({
      message: `Successfully added ${result.count} grocery items to your inventory!`,
      count: result.count,
    });
  } catch (error) {
    console.error("Error seeding items:", error);
    return NextResponse.json(
      { error: "Failed to seed grocery items" },
      { status: 500 }
    );
  }
}
